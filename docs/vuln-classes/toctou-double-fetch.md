...
Exploiting TOCTOU requires racing two threads: one invokes the syscall or IOCTL, while a second rapidly flips the validated field between a safe value and a malicious value. The race window is typically small (a few instructions between the check and the use), so exploitation requires many attempts. Modern multi-core systems make this reliable enough for practical use -- success rates of 10-50% per attempt are common, and the attack can be retried indefinitely from user mode.

Thread pinning to the same CPU core (via `SetThreadAffinityMask`) ensures tight scheduling interleaving. `NtSuspendThread` and `NtResumeThread` can precisely stall the victim thread between the check and the use. Page-fault-based stalling (placing the user buffer at a page boundary where the second access causes a soft page fault) widens the window to microseconds.
