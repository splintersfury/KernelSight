...
The information disclosure variant is the most common. When a driver allocates an output buffer, partially fills it with response data, and copies the entire buffer back to user mode, the unfilled portions contain stale kernel memory. This stale data frequently includes kernel pointers (from previous stack frames or pool allocations), which defeats KASLR. Leaking a single kernel pointer gives the base address of `ntoskrnl.exe` or a driver, enabling subsequent exploitation stages that require kernel address knowledge.
...
The control flow variant is less common but more dangerous. If an uninitialized stack variable is used as a function pointer, loop bound, or branch condition, an attacker who can influence residual stack contents (through prior system calls that leave specific values on the kernel stack) may control the uninitialized value. This technique ("stack variable spraying") involves making specific system calls before the vulnerable call to deposit controlled values at the right stack offsets. Pool variants use heap grooming to influence what data remains in a freshly allocated (but unzeroed) pool block.
...
Information disclosure via uninitialized memory is typically a KASLR bypass primitive. An IOCTL triggers the partial output buffer fill, the kernel pointers are read back from the uninitialized portion, and the kernel base address is computed. This is a precursor for a second vulnerability that requires kernel address knowledge (e.g., ROP chains or specific object address targeting).

Control flow exploitation through uninitialized memory is harder but has been demonstrated. The attacker must determine which prior kernel operations leave specific values at the stack offset where the uninitialized variable resides, requiring understanding of the kernel's call stack layout and finding a "stack spraying" primitive that writes a controlled value to the right offset. Pool variants use heap grooming: free a specifically-crafted allocation, then trigger the vulnerable path to allocate from the same pool with the same size, inheriting the stale data.

`ExAllocatePool2` (introduced in Windows 10 2004) zeroes allocations by default unless the caller opts out with `POOL_FLAG_UNINITIALIZED`, reducing pool-based variants. Stack-based variants remain common because stack memory is never automatically zeroed for performance reasons.
