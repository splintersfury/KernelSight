...
UAF bugs in the Windows kernel are especially dangerous because the pool allocator's behavior is sufficiently deterministic for reliable exploitation. The kernel's pool allocator groups allocations by size class, so an attacker who frees an object of size N and immediately allocates a controlled buffer of the same size N has a high probability of reclaiming the exact same memory. This makes UAF exploitation significantly more reliable than in typical user-mode heap exploits where allocator randomization and metadata integrity checks are more aggressive.
...
The `afd.sys` (Ancillary Function Driver for WinSock) and `win32k` subsystem are historically the most prolific sources of UAF vulnerabilities in Windows. `afd.sys` manages complex asynchronous I/O state for network sockets, and `win32k` manages deeply nested USER/GDI object hierarchies with callback reentrancy. Both drivers have extensive multi-threaded codepaths where object lifetime management is inherently difficult, leading to a recurring pattern of reference count and synchronization bugs.
...
UAF exploitation follows a predictable three-step pattern. First, the attacker triggers the free of the target object. Second, the attacker reclaims the freed memory by spraying allocations of the same size with controlled content -- common spray objects include pipe attributes, named pipe queue entries, extended attributes, or other user-controllable kernel allocations. Third, the attacker triggers the stale pointer dereference, causing the driver to interpret the attacker's sprayed data as the original object's fields.

The most powerful UAF exploitation occurs when the original object contains function pointers or vtable pointers. By placing a fake vtable pointer in the sprayed data, the attacker redirects a virtual function call to arbitrary code. With kCFI (kernel Control Flow Integrity) on modern Windows, vtable hijacking is harder, but data-only attacks remain viable -- corrupting a pointer to redirect a subsequent read/write operation to an attacker-chosen address.

The reliability of UAF exploitation in the kernel is generally high because the pool allocator's size-class bucketing makes reclamation predictable. The main challenge is timing: the attacker must complete the spray between the free and the stale use. For race-based UAFs, this window may be narrow, but for reference count bugs where the free is deterministic, the spray can be performed at leisure before triggering the stale access.
