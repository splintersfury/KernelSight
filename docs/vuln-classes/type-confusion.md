...
This vulnerability class is common in drivers that handle multiple object types through a common dispatch path. Kernel streaming drivers (`ks.sys`, `mskssrv.sys`) manage multiple object types (filters, pins, nodes) that share common handle infrastructure. If an IOCTL handler accepts a handle and dereferences it without verifying the object type via `ObReferenceObjectByHandle` with the correct `ObjectType` parameter, a handle to the wrong object type can be passed, causing the driver to misinterpret its fields.
...
A related pattern occurs in the `win32k` subsystem, where GDI and USER objects share handle table infrastructure but have different internal layouts. Confusing a PALETTE handle for a BITMAP handle, or a WINDOW handle for a MENU handle, causes the driver to interpret the object's body with the wrong field layout. The `win32k` subsystem has been a recurring source of type confusion vulnerabilities due to its large number of polymorphic object types.
...
Type confusion gives control over how the kernel interprets memory. If a data field in type A overlaps with a function pointer in type B, the value controls what code gets executed. If a size field in type A overlaps with a pointer field in type B, it controls the target of a memory operation. The exploitation path depends on which fields overlap between the confused types.

In practice, two object types are identified where one has attacker-controllable fields at offsets that correspond to security-relevant fields (function pointers, sizes, addresses) in the other type. An object of the controllable type is created, and the driver is tricked into treating it as the other type -- typically by passing the wrong handle type to an IOCTL or by manipulating object state to cause a code path to use the wrong cast.

Type confusion bugs often provide clean primitives. Unlike buffer overflows that require heap grooming and produce broad corruption, a well-exploited type confusion gives precise control over specific fields, making the exploit more reliable and less likely to cause collateral corruption that triggers a bugcheck.
