...
The TCP/IP stack parses complex protocol headers from untrusted network data and carries the most exposure. IPv6 processing stands out due to the extension header chain model, where each header contains a "next header" field pointing to the subsequent header type, and the stack must parse variable-length options within each extension header. CVE-2024-38063 demonstrated that an integer underflow in IPv6 packet reassembly length calculation could be triggered remotely by sending crafted IPv6 packets, achieving remote code execution without user interaction. IPv6 is enabled by default on all modern Windows installations, and the attack requires no authentication.

The HTTP protocol stack (`http.sys`) adds another remotely reachable parsing layer. It processes HTTP request headers, trailers, and content-encoding in kernel mode for performance, so malformed HTTP requests directed at any service using the HTTP Server API can trigger kernel vulnerabilities. CVE-2022-21907 in HTTP trailer parsing allowed remote code execution via a crafted HTTP request. `http.sys` is used by IIS, WinRM, SSDP, and many other Windows components, giving it broad exposure on server systems.

Locally, the Winsock ancillary function driver `afd.sys` handles socket operations from user mode and has been a recurring source of local privilege escalation vulnerabilities. It processes IOCTL requests for socket creation, binding, connection, and data transfer, and its internal buffer management involves chain operations on I/O request structures. The complex state management and direct user-mode reachability make `afd.sys` one of the most frequently patched Windows kernel components.
