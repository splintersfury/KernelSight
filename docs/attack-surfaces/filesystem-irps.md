...
- **CLFS log fuzzing**: Create malformed BLF files by corrupting base record headers, container contexts, and client contexts, then trigger parsing via `CreateLogFile`. The format complexity makes CLFS a productive fuzzing target.
